export const courseNodes = [
  // PLANNED
  {
    id: '6.172',
    new_id: '6.1060',
    label: '6.172',
    name: 'Software Performance Engineering',
    description: '',
    semester: 'Fall ‘23',
    icon: 'node-icons/software.svg',
    data: {
      category: 'Software',
    },
  },
  {
    id: '6.039',
    new_id: '6.1810',
    label: '6.039',
    name: 'Operating System Engineering',
    description: '',
    semester: 'Fall ‘23',
    icon: 'node-icons/systems.svg',
    data: {
      category: 'Systems',
    },
  },
  {
    id: '6.S953',
    new_id: '6.S953',
    label: '6.S953',
    name: 'Machine Learning for Drug Discovery',
    description: '',
    semester: 'Fall ‘23',
    icon: 'node-icons/ai.svg',
    data: {
      category: 'ML/AI',
    },
  },

  // PAST
  {
    id: '6.824',
    new_id: '6.5840',
    label: '6.824',
    name: 'Distributed Computer Systems Engineering',
    description: '',
    semester: 'Spring ‘23',
    icon: 'node-icons/systems.svg',
    data: {
      category: 'Systems',
    },
  },
  {
    id: '6.046',
    new_id: '6.1220',
    label: '6.046',
    name: 'Design and Analysis of Algorithms',
    description: '',
    semester: 'Spring ‘23',
    icon: 'node-icons/theory.svg',
    data: {
      category: 'Theory',
    },
  },
  {
    id: '6.033',
    new_id: '6.1800',
    label: '6.033',
    name: 'Computer Systems Engineering',
    description: '',
    semester: 'Spring ‘23',
    icon: 'node-icons/systems.svg',
    data: {
      category: 'Systems',
    },
  },
  {
    id: '6.819',
    new_id: '6.8301',
    label: '6.819',
    name: 'Advances in Computer Vision',
    description: '',
    semester: 'Spring ‘23',
    icon: 'node-icons/ai.svg',
    data: {
      category: 'ML/AI',
    },
  },
  {
    id: '21G.906',
    new_id: '21G.906',
    label: '21G.906',
    name: 'Korean 6',
    description: '',
    semester: 'Spring ‘23',
    icon: 'node-icons/language.svg',
    data: {
      category: 'Global Languages',
    },
  },

  {
    id: '6.042',
    new_id: '6.1200',
    label: '6.042',
    name: 'Mathematics for Computer Science',
    description: '',
    semester: 'Fall ‘22',
    icon: 'node-icons/theory.svg',
    data: {
      category: 'Theory',
    },
  },
  {
    id: '6.004',
    new_id: '6.1910',
    label: '6.004',
    name: 'Computation Structures',
    description: '',
    semester: 'Fall ‘22',
    icon: 'node-icons/systems.svg',
    data: {
      category: 'Systems',
    },
  },
  {
    id: '6.811',
    new_id: '6.4530',
    label: '6.811',
    name: 'Principles and Practice of Assistive Technology',
    description: '',
    semester: 'Fall ‘22',
    icon: 'node-icons/software.svg',
    data: {
      category: 'Software',
    },
  },
  {
    id: '6.864',
    new_id: '6.8610',
    label: '6.864',
    name: 'Quantitative Methods for Natural Language Processing',
    description: '',
    semester: 'Fall ‘22',
    icon: 'node-icons/ai.svg',
    data: {
      category: 'ML/AI',
    },
  },
  {
    id: '18.600',
    new_id: '18.600',
    label: '18.600',
    name: 'Probability and Random Variables',
    description: '',
    semester: 'Fall ‘22',
    icon: 'node-icons/theory.svg',
    data: {
      category: 'Theory',
    },
  },
  {
    id: '21G.905',
    new_id: '21G.905',
    label: '21G.905',
    name: 'Korean 5',
    description: '',
    semester: 'Fall ‘22',
    icon: 'node-icons/language.svg',
    data: {
      category: 'Global Languages',
    },
  },

  {
    id: '6.031',
    new_id: '6.1020',
    label: '6.031',
    name: 'Elements of Software Construction',
    description: '',
    semester: 'Spring ‘22',
    icon: 'node-icons/software.svg',
    data: {
      category: 'Software',
    },
  },
  {
    id: '6.036',
    new_id: '6.3900',
    label: '6.036',
    name: 'Introduction to Machine Learning',
    description: '',
    semester: 'Spring ‘22',
    icon: 'node-icons/ai.svg',
    data: {
      category: 'ML/AI',
    },
  },
  {
    id: '8.02',
    new_id: '8.02',
    label: '8.02',
    name: 'Electricity and Magnetism',
    description: '',
    semester: 'Spring ‘22',
    icon: 'node-icons/systems.svg',
    data: {
      category: 'Systems',
    },
  },
  {
    id: '21G.904',
    new_id: '21G.904',
    label: '21G.904',
    name: 'Korean 4',
    description: '',
    semester: 'Spring ‘22',
    icon: 'node-icons/language.svg',
    data: {
      category: 'Global Languages',
    },
  },

  {
    id: '6.009',
    new_id: '6.1010',
    label: '6.009',
    name: 'Fundamentals of Programming',
    description: '',
    semester: 'Fall ‘21',
    icon: 'node-icons/software.svg',
    data: {
      category: 'Software',
    },
  },
  {
    id: '6.006',
    new_id: '6.1210',
    label: '6.006',
    name: 'Introduction to Algorithms',
    description: '',
    semester: 'Fall ‘21',
    icon: 'node-icons/theory.svg',
    data: {
      category: 'Theory',
    },
  },
  // {
  //   id: 'CC.110',
  //   new_id: 'CC.110',
  //   label: 'CC.110',
  //   name: 'Becoming Human: Ancient Greek Perspectives on the Good Life (Concourse)',
  //   description:
  //     '',
  //   semester: 'Fall ‘21',
  // },
  // {
  //   id: 'CC.5111',
  //   new_id: 'CC.5111',
  //   label: 'CC.5111',
  //   name: 'Principles of Chemical Science (Concourse)',
  //   description:
  //     '',
  //   semester: 'Fall ‘21',
  // },

  // Other
  {
    id: '18.06',
    new_id: '18.06',
    label: '18.06',
    name: 'Linear Algebra',
    description: '',
    semester: 'Prior Credit',
    icon: 'node-icons/ai.svg',
    data: {
      category: 'ML/AI',
    },
  },
  // {
  //   id: '18.02',
  //   new_id: '18.02',
  //   label: '18.02',
  //   name: 'Multivariable Calculus',
  //   description:
  //     '',
  //   semester: 'Prior Credit',
  // },
  // {
  //   id: '18.01',
  //   new_id: '18.01',
  //   label: '18.01',
  //   name: 'Calculus',
  //   description:
  //     '',
  //   semester: 'Prior Credit',
  // },
  // {
  //   id: '8.01',
  //   new_id: '8.01',
  //   label: '8.01',
  //   name: 'Mechanics',
  //   description:
  //     '',
  //   semester: 'Prior Credit',
  // },
  // {
  //   id: '6.0001',
  //   new_id: '6.100A',
  //   label: '6.0001',
  //   name: 'Introduction to Computer Science and Programming in Python',
  //   description:
  //     '',
  //   semester: 'Prior Credit',
  // },
  // {
  //   id: '7.012',
  //   new_id: '7.012',
  //   label: '7.012',
  //   name: 'Introductory Biology',
  //   description:
  //     '',
  //   semester: 'Prior Credit',
  // },

  // {
  //   id: '',
  //   new_id: '',
  //   label: '',
  //   name: '',
  //   description:
  //     '',
  //   semester: '',
  // },
];
